	...
	<div class="container">
		<div class="row">
			...
			<table class="table table-striped"
				...
				<tbody>
					<%
						Bbs3DAO bbs3DAO = new Bbs3DAO();
						ArrayList<Bbs3> list = bbs3DAO.getList(pageNumber);
						ArrayList<Bbs3> listAll = bbs3DAO.getAllList();
					...
					<tr>
						...
						%>
						<td><a
							href="view3.jsp?bbsID=<%=list.get(i).getBbsID()%>&modNum=<%=0%>#<%=0%>">[BEST] <%=list.get(i).getBbsTitle().replaceAll(" ", "&nbsp").replaceAll("<", "&lt")
						...
						%>
						<td><a
							href="view3.jsp?bbsID=<%=list.get(i).getBbsID()%>&modNum=<%=0%>#<%=0%>"><%=list.get(i).getBbsTitle().replaceAll(" ", "&nbsp").replaceAll("<", "&lt")
						.replaceAll(">", "&gt").replaceAll("\n", "<br>")%></a></td>
						<%
							}
						%>
			...
			<%
				if (pageNumber != 1) {
			%>
			<a href="bbs3.jsp?pageNumber=<%=pageNumber - 1%>"
				class="btn btn-success btn-arrow-left">이전</a>
			<%
				}
				if (bbs3DAO.nextPage(pageNumber + 1)&& k-OrderNum != 0) { //다음 페이지가 존재한다면
			%>
			...
			%>
			<a href="write3.jsp" class="btn btn-primary pull-right">글 작성</a>
		</div>
	</div>
